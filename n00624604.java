...
class Prompt{
	...
	Prompt(){
		...
		try{
		String x = JOPtionPane.showMessageDialog("ENTER TWO INTS, THE FIRST BEING THE MAXIMUM VALUE OF
					THE INTS THAT ARE INSERTED AND THE SECOND BEING THE
					MAXIMUM VALUE OF THE INTS THAT ARE SEARCHED FOR: BUT IN NO CASE SHOULD EITHER EXCEED 2**31-1");
					
		StringTokenizer tokenizer = new StringTokenizer(x);
		
		maxintsinserted = tokenizer.nextToken();
		maxintsearch	= tokenizer.nextToken();
		}
		
		catch(Exception e){
			JOptionPane.showErrorDialog("Failed to parse ints (did you enter them correctly?)");
		}
				
	}
	
	public int getMaxInsert(){
		return(maxintinserted);
	}
